...
interface IncomingWSMessage {
  type: 'draft_ready' | 'message_finalized' | 'voice_finalized' | string;
  id: string;
  sender_email: string;
  text?: string;
  translated_text?: string | null;
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  connect: (roomId, token, myEmail) => {
    ...
    const ws = new WebSocket(`${WS}/${roomId}?token=${encodeURIComponent(token)}`);
    ...
    ws.onmessage = (e) => {
      ...
      try {
        ...
        console.warn('[chatStore] Received non-JSON WS message:', e.data);
        return;
      }
