...
export default function LiveRoomPage() {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${WS_BASE_URL}/api/v1/webrtc/ws/live/${roomId}?token=${encodeURIComponent(token)}`);
    wsRef.current = ws;

    ws.onmessage = async (e) => {
      let msg: { type?: string; candidate?: RTCIceCandidateInit; text?: string; tts_url?: string; detail?: string };
      try {
      ...
      if (msg.type === 'answer') {
        await pcRef.current?.setRemoteDescription(new RTCSessionDescription(msg as RTCSessionDescriptionInit));
      } else if (msg.type === 'candidate' && msg.candidate) {
        // Remote ICE candidates from the server were never applied in the
        // original code — without this, the connection can silently fail
        // to establish on many networks (especially non-local ones).
        try {
        ...
        if (audioRef.current && msg.tts_url) {
          audioRef.current.src = `${MINIO_URL}/${msg.tts_url}`;
          audioRef.current.play().catch((err) => {
            // Autoplay can be blocked by the browser; not fatal, user can hit play manually.
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white p-8">
      ...
      <button onClick={handleEnd} className="mt-8 bg-red-600 hover:bg-red-700 px-6 py-2 rounded">
        End
      </button>
    </div>
  );
